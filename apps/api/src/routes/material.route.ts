...
export function materialRoutes(deps: AppDependencies): Hono {
  const repo = new MaterialRepo(deps.db)
  const codeRepo = new CodeSequenceRepo(deps.db)
  const router = new Hono()

  router.get("/", async (c) => {
    const query = materialQuerySchema.parse(c.req.queries())
  ...
  router.post("/", async (c) => {
    const body = createMaterialSchema.parse(await c.req.json())
    const code = await nextCode(codeRepo, "materials", "MAT")
    const row = await repo.insert({ ...body, code })
    return c.json(successResponse(row), 201)
  })

  router.put("/:id", async (c) => {
    const body = updateMaterialSchema.parse(await c.req.json())
  ...
  router.delete("/:id", async (c) => {
    const row = await repo.findById(c.req.param("id"))
    if (row === null) {
      return c.json(successResponse(null, "物料不存在"))
    }
